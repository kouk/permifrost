...
class TestSnowflakeConnector:
    ...
    def test_uses_oauth_if_available(selfself, mocker, snowflake_connector_env):
        ...
        sqlalchemy.create_engine.assert_called_with("snowflake://TEST:@TEST/?authenticator=oauth&token=TEST&warehouse=TEST")

    def test_uses_username_password_by_default(selfself, mocker, snowflake_connector_env):
        mocker.patch("sqlalchemy.create_engine")
        SnowflakeConnector()
        sqlalchemy.create_engine.assert_called_with("snowflake://TEST:TEST@TEST/TEST?role=TEST&warehouse=TEST")
